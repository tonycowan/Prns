...
use personal_hopspot_core::{
    ble_interface_menu_details, expand_face_rgba, face_64x128, interface_mode_slot,
    load_host_interface_modes, save_host_interface_modes, snapshots_to_cards, AccessPointState,
    Card, CardActivityTracker, InputEvent, InterfaceModeTable, PowerSnapshot, ScreenContent,
    UiAction, UiConfiguration, UiNotice, UiState, UserBlanking, INTERFACE_MODE_STORAGE,
    MOBILE_RGBA_BYTES,
};
...
use crate::engine::{
    classify, configured_storage_dir, interface_snapshots, sleep_interfaces, toggle_interface,
    wake_interfaces,
};
...
impl HopspotFace {
    ...
    fn bind_modes_if_ready(&mut self) {
        ...
        let Some(storage_dir) = configured_storage_dir() else {
            return;
        };
    ...
    pub fn post_input(&mut self, event: InputEvent) -> UiAction {
        ...
        match action {
            ...
            UiAction::SetInterfaceMode { slot, selection } => {
                ...
                let notice = match configured_storage_dir() {
                    Some(storage_dir) => {
